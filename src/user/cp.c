...
void cp(char (*parsed_args)[128]){
        ...
        if(dir_table.table[i].user_attribute == UATTR_NOT_EMPTY){
            ...
            if(strcmp(dir_table.table[i].name, src_directories[src_dir_count - 1], 8) == 0){
                ...
                for(unsigned j = 0; j < strlen(src_directories[src_dir_count - 1]); j++){
                    if(is_ext){
                        ext[k] = src_directories[src_dir_count - 1][j];
                        k++;
                ...
                if(strcmp(dir_table.table[i].ext, ext, 3) == 0){
                    found = true;
                    break;
                } 
            }
    ...
    for(int i = 0; i < dest_dir_count; i++){
    ...
    memcpy(request.name, src_directories[src_dir_count-1], 8);
